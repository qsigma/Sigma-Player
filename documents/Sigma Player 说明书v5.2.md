# Sigma Player 说明书
适用于 Sigma Player 5.2 和 Sigma Player Pro 5.2
...
### 注册与登录交响诗班账户
Sigma Player 的 [交响诗班网络曲库](#### 选择琴谱文件) 和 [联机合奏](### 联机合奏) 服务需要登录交响诗班账号。
...
### 更新版本
Sigma Player Pro 4.2、Sigma Player 4.3 及以上版本配备在线热更新功能，软件会在运行时检测是否有更新可用。如果检测到新版本，软件左上角会出现如下图标：
...
- 为了保证软件的正常功能运作，软件可能会**读取，修改或删除**你的当前或历史版本 Sigma Player 文件夹中的琴谱、图标、音源等文件。
- 在自动演奏中，软件会操纵你的**键盘**进行**虚拟输入**。
- Sigma Player 不会修改游戏内数据，因此不会导致任何进行自动演奏的游戏封号，请你放心使用。
- > [按键映射](#按键映射)功能*不会*修改你的实际键位映射，因此只会在自动输出或软件内手弹演奏时捕获并映射你的按键，无法在游戏中将你的手弹演奏进行映射。
### 你的支持
...
- 顶部的常驻 UI，包含导入琴谱和几个使用软件时的图标按键。
 ...
 <img width="377" alt="image" src="https://github.com/user-attachments/assets/3037e4a4-d364-4d07-ba61-db171dc22a11" />
  ...
  - **登录**：点按后登录交响诗班账户。在未登录时，图标呈现灰色：<img width="16" height="17" alt="image" src="https://github.com/user-attachments/assets/a5885d79-61ae-452b-b522-2cc96be4b16b" />。登录后，图标呈现绿色，此时再次点击，可以退出登录。
 
  <img width="226" height="288" alt="image" src="https://github.com/user-attachments/assets/486e8a9c-5456-42bc-917a-d39d65484716" />


  - 导入**文件**：从本地文件导入`.mid`或`.txt`文件。
  - 导入**在线曲库**：从[在线曲库](#选择琴谱文件)导入歌曲。
...
- **从在线曲库中选择**。点击软件右上角的<img width="18" alt="image" src="https://github.com/user-attachments/assets/3f633ef2-b95b-4a39-972d-a1d62604372b" />，以打开在线曲库。
  - **自建在线曲库**。自建在线曲库会指向此 GitHub 仓库保存的曲谱。你可以在搜索栏搜索关键字，然后在弹出的结果中选择下载或者导入文本框。你还可以点击【给我惊喜】，随机获取十条结果。
  ...
  - **交响诗班网络曲库**。交响诗班网络曲库会指向交响诗班中的网络曲库，需要登录交响诗班账号使用。你同样可以在里面进行搜索。Sigma Player尊重交响诗班的运营理念，交响诗班的搜索结果导入支持播放（播放相关功能与midi文件地位完全相同，支持联机合奏、midi播放配置、快捷键控制、瀑布流等功能），不支持转谱、加入播放列表、下载等功能。
...
### 按键映射
你可以在音频中枢标签页新建或者切换一个按键映射方案。按键映射每一条记录分三列内容，每一条记录的格式形如 `字符-按键-音调`。
点按、长音自动演奏会调用 `字符-按键` 映射，内置诗琴手弹演奏会调用 `按键-音调` 映射；内置诗琴自动演奏会调用 `字符-音调` 映射。

<img width="361" alt="image" src="https://github.com/user-attachments/assets/3344bf6d-e95b-488d-97bd-abc3175f1216" />

<img width="401" height="532" alt="image" src="https://github.com/user-attachments/assets/fddcd6cc-8ece-4f73-a1fa-aa4ade7c658c" />


- 按键映射只对自动演奏的三个模式和内置诗琴生效，不会修改真实键盘的按键映射。
- 输入字符可以是任意 1 长度的**小写**字母或汉字或字符，或在它的前面添加 #。字符不可以包含：`+ - = · 《》 【】 ▲`；
- 冲突的输入字符会标红提示；
- 要设置 "映射为" 的按键，点按按钮后，按钮会变绿；此时在键盘上按下你想要修改的按键即可。注意，如果你设定了回车键，需要移开鼠标后再按一次。

#### 黑键支持
在 Sigma Player 5.0 版本，自动演奏、Midi 转谱、Midi 播放都支持了读取黑键。Sigma Player 谱中的黑键和普通键位的表示汇总如下：
...
点击最右侧的按键，可以选择在当前定调下，黑键音符的处理方法。可以在黑键靠上、黑键靠下、黑键略去、黑键保留中选择。
点击右侧倒数第二个数字，会打开 Midi 播放控制台，里面包含定调、黑键方案、音轨选择等全面的控制支持。

 <img width="299" height="506" alt="image" src="https://github.com/user-attachments/assets/1b1e70aa-2205-4363-8fdc-7c93edda422b" />

- 你可以通过 `shift/ctrl` 来选择音轨进行输出。一般来说 Midi 文件会在第一轨写入元数据，因此尽管它的长度可能为 0， 也请将它选中。
- 你可以通过滑动条，或者 `+1/-1/+12/-12` 按钮来改变定调；如果我们的算法识别到了最佳八度/最佳调式/原始调式，则对应的标签会亮起；你也可以点击标签来自动调整到最佳的方案。
- 底部的画布展示了 Midi 文件不同八度的音高分布，以及当前音调的黑键个数。这是一个静态的仅供参考的页面。
- 你可以选择【启用 Sigma 自适应定调算法】，这会将 Midi 文件超出音域的按键进行合理的音域压缩，以在三个八度的游戏乐器中获得最佳听感。
...
### 联机合奏
在 Sigma Player 5.2 版本，软件接入了交响诗班的联机合奏服务，需要你登录交响诗班账号以使用。联机合奏目前支持【自动演奏】模式，由房主将曲谱推送到房间中，所有人同时在游戏中开始演奏曲谱中的各条音轨。

联机合奏的入口在【音频中枢】底部。登录交响诗班账号后你可以选择【创建房间】或者【加入房间】。如果选择【加入房间】，请向你的琴友索要房间号码。联机面板在窄UI模式下出现在音频中枢的中下部，宽UI模式下出现在音频中枢的右部，与[歌单连播](###歌单连播)共享空间。

<img width="244" height="244" alt="0dd359aa21b342f4a9cfd771cf7e9429" src="https://github.com/user-attachments/assets/bc27c2bd-1001-4c41-9560-58751b41624d" />

创建房间的人自动成为房主，加入房间的人为普通成员。

<img width="302" height="337" alt="a2f71f47f9eeffcfb0ed414b625ee214" src="https://github.com/user-attachments/assets/b9fbdf1d-b8df-4cb2-8f03-4306b21bc3a3" />

- **成员与房间管理**：房主有踢出成员的权限，成员亦可自行退出。房主自行退出房间后，整个房间将直接解散。
- **推送曲目**：房间的所有人都有推送曲目的权限。在 Sigma Player 端，你执行的导入文件操作会自动将文件推送到房间中，这包括：从下拉框选择文本谱，打开本地的 txt/mid 文件，从自建在线曲库导入 midi 文件，从交响诗班在线曲库中导入 midi 文件，将本地 txt/mid 文件拖拽到软件中，歌单连播模式下自动切歌等等。注意，自建在线曲库的 txt 文件不会自动导入，因为其中并不全是 Sigma-呱呱谱。另外，你还可以点击联机 UI 中的【推送】按钮，选择导入当前文本框的内容或者当前导入的 mid 文件。
  
<img width="205" height="208" alt="c4193b27fede4bae0bce7ce39751f412" src="https://github.com/user-attachments/assets/11a81292-3a03-4fb1-8063-92955bbba528" />
- **演奏管理**：联机合奏中支持开始和结束两个信号。开始只能由房主开启。开启后，软件的【2】标签将会高亮变绿，此时软件会开始约7秒的倒计时，请在时间内将鼠标焦点切至游戏乐器界面。在 Sigma Player 中，开启就是经典的按下【1】按键。所有成员都可以自行按下【2】退出演奏，其中房主按下【2】后会发送终止演奏的指令，此时全部成员的演奏都会停止。联机合奏不支持暂停键。
> 在进入联机状态后，底部的点按/长音/音频状态栏会被禁用，且【1】检测全程开启。如果你不想误触，请打开【alt】锁定，用【alt+1】来触发。
- **演奏参数**：联机合奏中可以修改的播放设置为音轨和延音模式。每个成员的这两个设置都可以在联机UI上看到。房主可以修改成员的这些数据，成员仅能修改自己的数据。在 Sigma Player 中，无法在播放中实时修改输出的音轨。在本地演奏中，用户自己的按键映射、瀑布流等依然可以自行启用。
- **聊天**：你可以用对话框与房间成员聊天。
...
- 如果在控制中心的输出模式选择了【音频】，且保持软件的页面在【音频中枢】页面，则可以将软件变成原琴模拟器。按下对应按键时，软件顶部的对应琴键会发生变色
  ...
  - 在 Sigma Player 5.0，我们支持了 SF2 音源文件。你可以自行导入 SF2 或传统音频音源文件。如果你的 SF2 文件中设置了延音，Sigma Player 可以在内置诗琴手弹演奏、流式编曲等地方正确地应用它们。
  ...
  - 对于 SF2 文件，【乐器配置】会显示当前 SF2 乐器中可用的音域分布。
...
- 在选择【音频】输出模式时，点击【瀑布流】按钮，可以在生成音频的同时在对应的键位生成瀑布流；这个效果同样对在软件中手弹演奏生效；仅SF2音源可以实现正常的瀑布流效果。
  ...
  <img width="658" alt="image" src="https://github.com/user-attachments/assets/40ec64fe-b9a0-47f6-a3f7-6b1c6d0fb99c" />
...
### 速度设定
速度设定用来规定上文中的 X 的值。在琴谱开头时，需要用`《》`标定演奏速度。书名号内的数值为 + 停顿的时间（以秒为单位），你可以用 60/bpm 进行简单的换算。
> 60/bpm 得到的换算结果可能与实际相差一个整数倍，取决于你选取哪个延时符号作为四分音符。

- **局部变速**：如果在演奏中途，节奏发生了变化，你可以用新的《》标记新的演奏速度。则此后的所有文本，都会按照这个新的《》进行演奏。
- **速度锁**：如果你的谱子没有标记速度，则会默认赋予 0.2 的速度。你可以在音频中枢调节这个默认值。
- **自动演奏中的临时速度变换**：【按键指示区】在 4 5 之间的数值指示了你在自动演奏时通过 4 5 得到的临时相对速度倍率，范围在 0.5~5 之间，它与原始速度是乘算关系，也即数值越小，停顿时间越小，演奏速度越快。在演奏中止后，这个倍率会回到 1.0。
### 其它符号的说明
...
### js 谱转 Sigma-呱呱谱
为了更好的拟合 js 中的整数值停顿时长，我们搭载了 Sigma New-Beat 2.0 节奏拟合算法。点击转换后，可以调节预期输出的精确度/简洁性权重，并查看实时效果。
...
### 余音谱
我们为「余音」乐器的制谱专门适配了余音谱制谱工具。它要求输入曲谱的拍号，并会进行键位调整、和弦调整等操作。
> 余音谱由伴仙雪糕开发，一只白空空策划，冬冬冬茗统筹，dr_sigma 整合。

---

> - Sigma Player 内置的其他输入输出组合欢迎用户自行探寻！这些功能也集成在[万向转谱](#万向转谱)中，帮助你实现定制的转谱效果。此外，Sigma Player 将继续推进琴谱格式的转换，有其他 up 主的琴谱转换需求的，在得到 up 授权之后欢迎与我交流！
> - 你还可以前往[aogus.tech](aogus.tech)以在线实现指尖谱的更多转谱功能。
...
#### 副文本框
点击【副文本框】按钮，即可弹出一个独立文本框，你可以用它自由放置文本。它支持自动保存、万向转谱快捷键、快捷输入、置顶等功能和选项。自动保存的内容存放在  `autosave/helpersave.txt` 中。
...
- (Pro 独享) 右击（或在触控板中双指轻点）【预览】，打开批量转换窗口。将想要转换的曲谱或者文件夹拖拽进弹出的窗口，即可批量转换。**注意：为了保证数据安全，避免可能的错误导致琴谱资源被全部破坏，请先备份后再进行批量转换。**

万向转谱提供了如下基础操作：

<img width="673" alt="image" src="https://github.com/user-attachments/assets/faf25b1e-140a-40e3-bb85-d2394508d593" />
<img width="247" height="387" alt="image" src="https://github.com/user-attachments/assets/e893ff52-017a-4d0f-a0b4-3d4322a77958" />
...
- 排序、换行：调用软件自带的功能模块[琴谱排序](#### 琴谱排序)和[自动换行](#### 自动换行)。
- 指定音轨：指定 Sigma-呱呱谱中的部分音轨保留，去除其他的音轨。
...
## 练习模式 
### 打开练习模式
Sigma player 3.0 加入了练习模式。练习模式的入口在音频中枢界面；打开后，会弹出练习模式的窗口，练习模式窗口处于置顶位置。主页面中文本框内的琴谱会被按换行符、变速符号《》进行切分，并将当前行和下一行分别填入练习模式的窗口内。通过这种方式，琴谱以歌词的形式呈现。
...
- 右侧四个按钮用来开启练习模式或者进行编辑：
  ...
  - **编辑**：点击后变紫，开启后编辑小窗中第一个区块内的琴谱可以在关闭小窗后更新到原始文本框中。
...
按【2】，中止，停留在当前句子；
按【4】，停止0.8s，然后滑动条中的相对速度减少0.1；
按【5】，停止0.8s，然后滑动条中的相对速度增加0.1。
```
...
## 流式编曲 
Sigma player 4.0 加入了流式编曲，是专业的原琴谱制作界面。本模块参考了[genshin music nightly](https://specy.github.io/genshinMusic/)网站的思路，并支持了大量的新功能。
### 界面概览与操作

<img width="700" alt="image" src="https://github.com/user-attachments/assets/c2fcb297-7275-46e2-9ff1-6638ad7952b8" />



- 流式编曲界面由文本框副栏、瀑布流和其他 UI 构成，文本框和瀑布流的占比可以通过中间的分界线改变；列宽可以动态调整。
- 点击【宽高】按钮，可以在滑动条中修改瀑布流的宽和高。例如下图：
...
- 在画布中点击，也可以依据点击的相对位置跳转到瀑布流中的位置。
  <img width="260" alt="image" src="https://github.com/user-attachments/assets/4694948f-1444-4f6a-b593-732cf0e72776" />
...
- **□自动**：勾选该按钮会使文本框变为不可编辑状态，且每 5 秒更新一次瀑布流的内容。选中状态下无法执行同步操作。
  ...
  > - 在 Sigma Player 4.33 版本中，我们支持了**精确位置双向同步**，让编谱体验再次上升一个台阶：点击文本框中的文本，会跳转到瀑布流的对应列；点击瀑布流的列，文本框中的对应文本也会显示为红色。你可以用这种方式更加快捷的定位到要进行修改的位置。
### 小节线
...
<img width="593" height="506" alt="image" src="https://github.com/user-attachments/assets/9b8c0c7e-745f-451a-8b84-1877b93eee5d" />
...
> 音域分布向你指示了当前 Midi 文件与原琴音域的适配程度。如果你发现整首曲子前后的音高相差太大，可以考虑修改选择范围，分批次、采用不同的定调方案将 Midi 文件逐次导入。你还可以选择【启用Sigma自适应定调算法】，来自动将超出音域的音进行压缩。
...
- **为音轨分别选择八度**：勾选这个选项后，你可以为 Midi 文件的每一个音轨分别制定八度的策略。只需要从右下角的下拉菜单选择音轨，然后单击移动框体到指定位置即可。你也可以分别选择每一条音轨是否输出。
- **黑键分布**：在一个八度中共有 12 个音调（do, 升 do, re, 升 re, mi, fa, 升 fa, so, 升 so, la, 升 la, ti）。游戏内乐器（除【老旧的诗琴】外）均为 C 调音源，也即不接受升的音/黑键。不过在软件内的SF2版本的音源中，你可以输出黑键（参见[内置诗琴](#内置诗琴))。为此，软件会在每一个调式下，计算此时落在黑键上的音符个数，并将它们用柱状图反映在软件右侧的绿色画布上。这个分布不会随着选取的调整而改变。
- **定调**：软件会依据黑键最小原则，选择一个调式，并将它标红。

  在其它调的位置单击一次，会使框体在不改变八度的情况下微调至新的调式。
  > 你可以简单地将八度选择与定调理解为粗准焦螺旋和细准焦螺旋。
### 黑键策略与节奏拟合策略
- **黑键策略**：对于当前调式下存在的黑键音符，通过【略去】【靠上】【靠下】【保留】进行处理。
  > 黑键的保留，详见[黑键支持](#黑键支持)。这个功能不对Midi转谱窗口的试听生效。
- **节奏拟合策略**：与 [js 谱转 Sigma-呱呱谱](#js-谱转-Sigma-呱呱谱)效果一致。采用 Sigma New-Beat 2.0 算法，拉动滑动条选择导出的曲谱更精细或是更简洁。
...
> - 如果我们发现 Sigma Player Pro 的用户有滥用软件的行为，会将对应的 MAC 地址上传到黑名单列表。Sigma Player Pro就有在线检测黑名单的机制。
